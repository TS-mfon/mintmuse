      ...
      <section className="workspace">
        <div className="control-rail">
          ...
          <div className="panel main-panel">
            ...
            {step === "preview" && profile && <PreviewStage profile={profile} followerDisplay={followerDisplay} followingDisplay={followingDisplay} bound={boundForProfile} onBind={bindHandle} onGenerate={generate} binding={bindingLocked} confirming={bindReceipt.isLoading} verified={Boolean(verificationSignature)} />}
...
function PreviewStage({ profile, followerDisplay, followingDisplay, bound, verified, onBind, onGenerate, binding, confirming }: { profile: XProfile; followerDisplay: number; followingDisplay: number; bound: boolean; verified: boolean; onBind: () => void; onGenerate: () => void; binding: boolean; confirming: boolean }) { return <div><h2>Signal report</h2><div className="profile-mini"><img src={profile.avatar || ""} alt="" /><div><strong>{profile.displayName || profile.handle}</strong><span>@{profile.handle}</span></div></div><p className="bio">{profile.bio || "Bio unavailable"}</p><div className="metric-grid"><Metric label="Followers" value={profile.followers == null ? "Unavailable" : followerDisplay.toLocaleString()} source="X profile" /><Metric label="Following" value={profile.following == null ? "Unavailable" : followingDisplay.toLocaleString()} source="X profile" /></div><div className="action-stack"><button className="btn-primary" disabled={!verified || binding || bound} onClick={onBind}>{bound ? "Handle bound ✓" : confirming ? "Confirming on X Layer…" : binding ? "Transaction submitted…" : verified ? "Bind verified handle" : "Verify bio first"}</button><button className="btn-ghost" disabled={!bound || binding} onClick={onGenerate}>{bound ? "Generate with GenLayer" : "Available after binding"}</button></div>{binding && !bound && <p className="pending-note" aria-live="polite">Do not submit again. MintMuse is tracking the current binding transaction until it confirms or fails.</p>}</div>; }
